...
def construir_espacio_latente(scores_afe, coords_acm, n_components=3):
    """AFE+ACM → estandarización → PCA para mejorar separación clusters."""
    ...
    log.info(f"  QuantileTransformer(normal) + PCA: {n_components} componentes → {var_exp:.1f}%")
    return X_pca, qt, pca
...
def evaluar_kmeans(X: np.ndarray, k_range=range(2, 8)) -> dict:
    """KMeans sobre espacio latente QuantileNormal+PCA — método principal."""
    resultados = {}
...
def evaluar_kprototypes(df, X_latente, k_range=range(2, 8)):
    ...
    for k in k_range:
        try:
            kp = KMeans(n_clusters=k, init="Cao", random_state=42,
                             n_init=3, verbose=0)
...
def imprimir_comparacion(ward_res, kmeans_res, dbscan_res,
                          k_opt_w, k_sec_w, k_opt_km, k_sec_km):
    sep = "=" * 65
    print(f"\n{sep}")
...
def guardar_todo(df, labels_w_opt, labels_w_sec, labels_km_opt, labels_km_sec,
                 k_opt_w, k_sec_w, k_opt_km, k_sec_km,
                 ward_res, kmeans_res, resumen_afe, mca, scaler, pca_model=None):
...
def run():
    ...
    log.info("=" * 55)
    X_latente, qt, pca_model = construir_espacio_latente(scores_afe, coords_acm, n_components=3)
    ...
    k_opt_km, k_sec_km = seleccionar_mejor_k(kmeans_res, "KMeans", k_min=2)
    k_opt_w,  k_sec_w  = seleccionar_mejor_k(ward_res,   "Ward",   k_min=2)
 
    imprimir_comparacion(ward_res, kmeans_res, dbscan_res,
                          k_opt_w, k_sec_w, k_opt_km, k_sec_km)
 
    labels_km_opt  = renombrar_por_incidencia(kmeans_res[k_opt_km]["labels"], df)
    labels_km_sec  = renombrar_por_incidencia(kmeans_res[k_sec_km]["labels"], df)
    ...
    for nombre, labels, k in [
        ...
        (f"Ward k={k_opt_w} (validación)",                    labels_w_opt,  k_opt_w),
    ]:
        counts = pd.Series(labels).value_counts().sort_index()
        total = len(labels)
        print(f"\n  {nombre}:")
    ...
    guardar_todo(df, labels_km_opt, labels_km_sec, labels_w_opt, labels_w_sec,
                 k_opt_km, k_sec_km, k_opt_w, k_sec_w,
                 kmeans_res, ward_res, resumen_afe, mca, locals().get("qt"), pca_model)
